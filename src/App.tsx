...
const FONTS = `
@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;1,400&family=Caveat:wght@400;600;700&family=Lato:wght@300;400;700&display=swap');
`;

const CSS = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    --cream: #FDF6EC; --warm-white: #FFFAF4; --amber: #D4863A;
    --amber-light: #F5C07A; --brown: #5C3D2E; --brown-light: #8B6347;
    --rose: #C9747A; --sage: #7A9E7E; --shadow: rgba(92,61,46,0.18); --paper: #FEF9F0;
  }
  ...
  .header-logo { font-family: 'Caveat', cursive; font-size: 2rem; font-weight: 700; color: var(--amber-light); line-height: 1; }
  .header-sub { font-size: 0.72rem; color: rgba(245,192,122,0.65); letter-spacing: 2px; text-transform: uppercase; margin-top: 2px; }
  .member-badge { background: rgba(245,192,122,0.15); border: 1px solid rgba(245,192,122,0.35); border-radius: 40px; padding: 6px 16px; color: var(--amber-light); font-family: 'Caveat', cursive; font-size: 1rem; display: flex; align-items: center; gap: 6px; }
  ...
  .welcome-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 5px; background: linear-gradient(90deg, var(--amber), var(--rose), var(--sage)); border-radius: 24px 24px 0 0; }
  ...
  .name-input { width: 100%; padding: 14px 20px; border: 2px solid rgba(212,134,58,0.25); border-radius: 12px; font-size: 1rem; background: var(--warm-white); color: var(--brown); outline: none; transition: border-color 0.2s; margin-bottom: 16px; }
  .name-input:focus { border-color: var(--amber); }
  .name-input::placeholder { color: rgba(92,61,46,0.35); }
  .btn-primary { width: 100%; padding: 14px; background: var(--amber); color: white; border: none; border-radius: 12px; font-size: 1rem; font-weight: 700; cursor: pointer; transition: background 0.2s; }
  .btn-primary:hover { background: #C07530; }
  ...
  .section-title { font-family: 'Playfair Display', serif; font-size: 1.4rem; color: var(--brown); margin-bottom: 20px; display: flex; align-items: center; gap: 10px; }
  .drop-zone { border: 2.5px dashed rgba(212,134,58,0.4); border-radius: 16px; padding: 40px 24px; text-align: center; cursor: pointer; transition: all 0.2s; background: rgba(245,192,122,0.05); position: relative; }
  .drop-zone:hover { border-color: var(--amber); background: rgba(245,192,122,0.12); }
  ...
  .preview-remove { position: absolute; top: 3px; right: 3px; background: rgba(92,61,46,0.85); color: white; border: none; border-radius: 50%; width: 20px; height: 20px; font-size: 11px; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .caption-input { width: 100%; padding: 12px 16px; border: 2px solid rgba(212,134,58,0.2); border-radius: 10px; font-size: 0.9rem; color: var(--brown); background: var(--warm-white); outline: none; margin-top: 16px; transition: border-color 0.2s; resize: none; }
  .caption-input:focus { border-color: var(--amber); }
  .upload-btn { margin-top: 16px; padding: 12px 32px; background: var(--amber); color: white; border: none; border-radius: 10px; font-size: 0.95rem; font-weight: 700; cursor: pointer; transition: background 0.2s; }
  .upload-btn:hover { background: #C07530; }
  ...
  .refresh-btn { background: rgba(212,134,58,0.12); border: 1px solid rgba(212,134,58,0.25); border-radius: 10px; padding: 8px 16px; color: var(--amber); font-size: 0.85rem; font-weight: 700; cursor: pointer; }
  .photo-grid { columns: 3; column-gap: 20px; }
  @media (max-width: 700px) { .photo-grid { columns: 2; } }
  @media (max-width: 440px) { .photo-grid { columns: 1; } }
  .polaroid { background: white; border-radius: 4px; padding: 10px 10px 40px; box-shadow: 0 6px 24px rgba(92,61,46,0.14); margin-bottom: 20px; break-inside: avoid; transition: transform 0.25s; cursor: pointer; }
  ...
  .lightbox-close { position: absolute; top: -16px; right: -16px; background: var(--brown); color: var(--amber-light); border: none; border-radius: 50%; width: 36px; height: 36px; font-size: 18px; cursor: pointer; display: flex; align-items: center; justify-content: center; }
...
const fakeStorage = {
  get: async (key: string) => (storage[key] ? { value: storage[key] } : null),
...
export default function App() {
  ...
  const loadPhotos = useCallback(async () => {
    setLoading(true);
    try {
      const s = (window as any).storage || fakeStorage;
      const index = await s.get('bajirav-photo-index', true);
      if (!index) {
        setPhotos([]);
        setLoading(false);
        return;
      }
      const ids = JSON.parse(index.value);
      const loaded: Photo[] = [];
      for (const id of ids.slice().reverse()) {
        try {
          const r = await s.get(`bajirav:${id}`, true);
  ...
  useEffect(() => {
    if (memberName) loadPhotos();
  }, [memberName, loadPhotos]);
  ...
  const handleFiles = async (files: File[]) => {
    for (const f of files) {
      ...
      setPreviews((p) => [
        ...p,
        { id: Date.now() + Math.random(), src: compressed },
      ]);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      for (const preview of previews) {
        ...
        await s.set(`bajirav:${id}`, JSON.stringify(photo), true);
        let index: string[] = [];
        try {
          const r = await s.get('bajirav-photo-index', true);
          if (r) index = JSON.parse(r.value);
        } catch {}
        index.push(id);
        await s.set('bajirav-photo-index', JSON.stringify(index), true);
      }
      setPreviews([]);
      setCaption('');
      showToast(
        `✨ ${previews.length} photo${previews.length > 1 ? 's' : ''} shared!`
      );
      await loadPhotos();
    } catch {
        ...
        <div className="app welcome-screen">
          <div className="welcome-card">
            ...
            <p className="welcome-desc">
              A shared album for the Bajirav family. Upload photos and everyone
              can see them instantly!
            </p>
            <input
              className="name-input"
              placeholder="Your name (e.g. Mom, Arjun, Dadi...)"
              value={nameInput}
            ...
            >
              Created by Adarsh C Koragaonkar
            </div>
          </div>
        </div>
      </>
      ...
      <div className="app">
        ...
        <div className="main">
          <div className="upload-section">
            ...
            >
              ...
              <div className="drop-sub">
                Supports JPG, PNG • Multiple photos okay
              </div>
            </div>
            {previews.length > 0 && (
              <>
                  ...
                  {previews.map((p) => (
                    <div className="preview-item" key={p.id}>
                      ...
                      <button
                        ...
                        onClick={() =>
                          setPreviews((ps) => ps.filter((x) => x.id !== p.id))
                        }
                      >
                        ✕
                      </button>
          ...
          <div className="footer">
            ...
            >
              💛 Made with love for the Bajirav Family
            </div>
            <div
              style={{
                fontSize: '0.75rem',
          ...
          <div className="lightbox" onClick={() => setLightbox(null)}>
            ...
            >
              ...
              <div className="lightbox-info">
                ...
                >
                  Shared by {lightbox.uploader} · {timeAgo(lightbox.ts)}
                </div>
              </div>
            </div>
          </div>
